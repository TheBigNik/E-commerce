...
export const getFeaturedProducts = async (req, res) => {
  try {
    const featuredProducts = await Product.find({ is_featured: true }).lean();
    if (!featuredProducts) {
      return res.status(404).json({ message: "No featured products found" });
    }
  } catch (error) {
    console.log("Error on getFeaturedProducts controller", error.message);
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
export const deleteProduct = async (req, res) => {
  try {
    await Product.findByIdAndDelete(res.params.id);
  } catch (error) {
